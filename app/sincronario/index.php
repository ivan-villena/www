<?php
  ...
  $App->rec['ope']['ini']['dia'] = [
    ...
    ".Doc::nav('bar',[
      'kin' => [ 'ide'=>"kin", 'ico'=>"", 'nom'=>"Sincrónico", 'des'=>"", 'htm'=>Dat::inf_pos("hol","kin",[ 
        'kin'=>$_kin = Hol::_('kin',$_hol->val['kin']),
        'sel'=>Hol::_('sel',$_kin->arm_tra_dia),
        'ton'=>Hol::_('ton',$_kin->nav_ond_dia)
      ])],
      'psi' => [ 'ide'=>"psi", 'ico'=>"", 'nom'=>"Cíclico", 'des'=>"", 'htm'=>Dat::inf_pos("hol","psi",[ 
  ...
  if( empty($Uri->cab) || empty($Uri->art) ){
    ...
    <?php
    $App->rec['ope']['med']['app_dat']['htm'] = ob_get_clean();
  ...
  // por articulo 
    ...
    ob_start();  
    // Tableros        
    if( in_array($Uri->cab, $ope_atr = ['kin','psi']) ){

      // genero datos
      $_hol->dat = Hol::val_dat($Uri->cab, $_hol->val);
      
      // operadores del tablero
      $tab_ide = "hol.{$Uri->cab}";
      
      if( !( $tab_ope =  Dat::get_est('hol',"{$Uri->cab}_{$Uri->art}",'tab') ) ) $tab_ope = [];
      ...
      $lis_ope = Dat::get_est("hol",$Uri->cab,'lis');
      ...
      $ope = Dat::$Tab['ope']['lis'];
      $App->rec['ope']['ini']['lis'] = [ 'ico'=>$ope['ico'], 'tip'=>"win", 'nom'=>$ope['nom'], 
        'htm'=>Dat::tab_ope('lis',"hol.{$Uri->cab}",$lis_ope) 
      ];

      // 3- imprimo tablero en página principal
      echo "
      <article>
        ".Hol::tab($Uri->cab, $Uri->art, $tab_ope, [
    ...
    // Contenido: bibliografia + codigo + kin planetario
      ...
      // cargo directorio y enlaces
      $_dir = $App->rec_dir();
      $_bib = SYS_NAV."sincronario/bibliografia/";

      // cargo indice
      $_nav = $App->rec['app']['nav'];

      // cargo módulos
      if( !empty( $rec = Arc::val("./app/$Uri->esq/$Uri->cab") ) ){

        require_once( $rec );
      }

      // imprimo articulo : html-php
      if( !empty( $rec = Arc::val($val = "./app/$Uri->esq/$Uri->cab/$Uri->art") ) ){
  ...
  $App->rec['cla']['app/sincronario'] = [];
  if( !empty($Uri->cab) ){ 
    // de contenido
    $App->rec['cla']['app/sincronario'] []= $Uri->cab;
    // de articulo
    if( !empty($Uri->art) ){
      $App->rec['cla']["app/sincronario/{$Uri->cab}"] = [ $Uri->art ];
    }
  }
