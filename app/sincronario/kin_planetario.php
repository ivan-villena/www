<?php  
  ...
  function dat_inf() : string {
    ...
    $esq = 'usu';
    $est = 'dat';
    $_kin = Hol::_('kin',$api_usu->kin);
    $_psi = Hol::_('psi',$api_usu->psi);
    $_ = "
    <form class='dat' data-esq='{$esq}' data-est='{$est}'>

      <fieldset class='doc_ren'>
        ".Doc::var('atr', [$esq,$est,$atr='nom'], [ 'val'=>$api_usu->$atr  ], 'eti')."
        ".Doc::var('atr', [$esq,$est,$atr='ape'], [ 'val'=>$api_usu->$atr  ], 'eti')."                              
      </fieldset>

      <fieldset class='doc_ren'>
        ".Doc::var('atr', [$esq,$est,$atr='mai'], [ 'val'=>$api_usu->$atr  ],'eti')."
        ".Doc::var('atr', [$esq,$est,$atr='fec'], [ 'val'=>$api_usu->$atr, 'ite'=>[ 'class'=>"tam-ini" ]  ], 'eti')."
      </fieldset>
  ...
  function dat_fic( array $ope = [], ...$opc ) : string {
    ...
    <section class='doc_inf ren esp-ara'>
      ...
      <div>
        <p class='tex-tit tex-3 mar_aba-1'>".Tex::let("$api_usu->nom $api_usu->ape")."</p>
        <p>".Tex::let($_fec->val." ( $api_usu->eda años )")."</p>
      </div>        
  ...
  function cic( string $tip = NULL, mixed $val = NULL ) : string {
    ...
    if( empty($tip) ){
      ...
      case 'ani':          
        // elimino previos
        $_ .= "DELETE FROM `usu_cic_ani` WHERE usu = $api_usu->ide;<br>";
        $_ .= "DELETE FROM `usu_cic_lun` WHERE usu = $api_usu->ide;<br>";
        // pido tránsitos
        foreach( Hol::val_cic( $api_usu->sin, 1, 52, 'not-lun') as $_cic_ani ){
  ...
  function cic_dat( string $fec = '' ) : array {
    ...
    $_['ani'] = Dat::get('usu_cic_ani',[ 
      'ver'=>"`usu`='{$api_usu->ide}' AND `fec` <= '".Fec::val_var( $_['hol']['fec'] )."'", 'ord'=>"`ide` DESC", 'lim'=>1, 'opc'=>"uni"
    ]);

    // busco transito lunar
    $_['lun'] = Dat::get('usu_cic_lun',[ 
      'ver'=>"`usu`='{$api_usu->ide}' AND `ani`={$_['ani']->ide} AND `fec` <= '".Fec::val_var( $_['hol']['fec'] )."'", 'ord'=>"`ani`, `ide` DESC", 'lim'=>1, 'opc'=>"uni" 
    ]);

    // calculo diario
    $_['dia'] = new stdClass;
    $_['dia']->kin = Hol::_('kin', intval($_['hol']['kin']) + intval($api_usu->kin) );
  ...
  function cic_nav( array $ope = [], ...$opc ) : string {
    ...
    foreach( Dat::get("usu_cic") as $_arm ){
      $_lis_cic = [];
      foreach( Dat::get('usu_cic_ani',[ 'ver'=>"`usu`='{$api_usu->ide}' AND `arm`=$_arm->ide", 'ord'=>"`ide` ASC" ]) as $_cic ){
        // ciclos lunares
        $_lis_lun = [];
        foreach( Dat::get('usu_cic_lun',[ 'ver'=>"`usu`='{$api_usu->ide}' AND `ani`=$_cic->ide", 'ord'=>"`ide` ASC" ]) as $_lun ){                            
