...
class Eje {
  ...
  // ejecuto funcion
  static val( $ide, $par, $eje ){
    let $_=[], $={};
    
    if( typeof($ide) == 'string' && Array.isArray($par) && typeof($eje) == 'function' ){  
      ...
      // armo peticion
      $_ = `${SYS_NAV}index.php?_=${JSON.stringify([ $ide, $par ])}`;  
