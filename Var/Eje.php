...
class Eje {
  ...
  static function fun( string $ide, ...$par ) : mixed {
    ...
    if( !empty( $dir = implode('/',$dir) ) ){

      if( file_exists($rec = "$dir.php") ){ 
    ...
    if( function_exists($ide) ){
      ...
      if( empty($dir) ) $dir = "el entorno global";

      $_ = "<p class='err'>{-_-}.err: No existe la función '{$ide}' en {$dir}...</p>";
    }
      ...
      if( in_array($met, get_class_methods($cla)) ){
        ...
        if( $sep == "::" ){
          ...
          if( strpos($ide,'$') !== FALSE ){
            ...
            $cla_ide = !empty($_cla['dir']) ? $_cla['dir']."/".$cla : $cla;

            $obj = Eje::cla( $cla_ide, ...$ini );
          }
  ...
  static function cla( string $ide, ...$ini ) : mixed {
