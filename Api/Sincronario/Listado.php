...
class Listado {
  ...
  static function factor_maya( string $ide, array $var = [], array $ele = [] ) : string {                    
    $_ = [];
    $lis_pos = 0;
    $lis_tip = "dep";
    $dat_esq = "hol";
    ...
    case 'ton':
      $var['lis'] = ['class'=>"ite"];
      foreach( Dat::_('hol.ton') as $Ton ){ $_ []= "
        ".Doc_Val::ima($dat_esq,"ton",$Ton,['class'=>"mar_der-1"])."
        <p>
          <n>".intval($Ton->ide)."</n><c>.</c> El Rayo de Pulsación ".preg_replace("/^(del|de la)/","$1<b class='ide'>",Tex::art_del($Ton->gal))."</b>
        </p>";
      }        
      break;
    ...
    case 'sel_cic_ser':
      ...
      foreach( Dat::_('hol.sel') as $Sel ){
        ...
        $_dir = Dat::_('hol.sel_cic_dir',$Sel->arm_raz); $_ []=

        Doc_Val::ima($dat_esq,"sel",$Sel,['class'=>"mar_der-1"])."

        <p><n>{$Sel->ide}</n><c>.</c> <b class='ide'>{$Sel->nom_may}</b><c>:</c> ".explode(' ',$_dir->nom)[1]."<c>.</c>
          <br>".Doc_Val::let($Sel->cic_ser_des)."
        </p>";
      }        
      break;
    ...
    case 'sel_cic_luz': 
      ...
      foreach( Dat::_('hol.sel') as $Sel ){
        ...
        $_dir = Dat::_('hol.sel_cic_dir',$Sel->arm_raz);                 
        
        $_ []= 

        Doc_Val::ima($dat_esq,"sel",$Sel,['class'=>"mar_der-1"])."

        <p>".explode(' ',$_dir->nom)[1]."<c>:</c> <b class='ide'>{$Sel->nom_may}</b><c>.</c>
          <br>".Doc_Val::let($Sel->cic_luz_des)."
        </p>";                
      }          
      break;
    // kin : katun del kin
    ...
    case 'kin_arm_sel':
      ...
      foreach( Dat::_('hol.sel') as $Sel ){ $_ [] = "

        ".Doc_Val::ima($dat_esq,"sel_arm_tra",$Sel,['class'=>"mar_der-2"])."

        <p>
          <b class='ide'>{$Sel->nom_may}</b><c>:</c> Katún <n>".(intval($Sel->ide)-1)."</n>
          <br>{$Sel->arm_tra_des}
        </p>";
      }
      break;
    ...
    case 'kin_cro_est':
      ...
      foreach( Dat::_('hol.kin_cro_est') as $_est ){
        $Sel = Dat::_('hol.sel',$_est->sel); 
        $_dir = Dat::_('hol.sel_cic_dir',$_est->ide); $_ []= 
        
        Doc_Val::ima($dat_esq,"sel",$Sel,['class'=>"mar_der-1"])."

        <p><n>".intval($Sel->ide)."</n><c>°</c> Signo<c>.</c> ".explode(' ',$_dir->nom)[1]."<c>:</c> <b class='ide'>{$Sel->nom_may}</b><c>.</c>
    ...
    case 'kin_cro_sel':
      foreach( Dat::_("hol.kin_cro_est") as $_est ){
        $Sel = Dat::_('hol.sel',$_est->sel); $htm = "
        <div class='-val'>
          ".Doc_Val::ima($dat_esq,"sel",$Sel,['class'=>"mar_der-2"])."
          <p>
            <b class='tit'>ESTACIÓN ".Tex::let_may(Tex::art_del("el {$_est->des_dir}"))."</b>
            <br>Guardían<c>:</c> <b class='ide'>{$Sel->nom_may}</b> <c>(</c> {$Sel->nom} <c>)</c>
          </p>
        </div>";
        $lis = [];
  ...
  static function encantamiento_del_sueño( string $ide, array $var = [], array $ele = [] ) : string {                    
    ...
    case 'ton_ond':
      ...
      // cargo valores
      foreach( ( $_dat = Obj::atr(Dat::_('hol.ton')) ) as $Ton ){
        $Ton->ima = [ 'htm'=>Doc_Val::ima($dat_esq,"ton",$Ton,[ 'class'=>"tam-5 mar-1" ]) ];
        $Ton->ide = "Tono {$Ton->ide}";
      }
      // cargo titulos
      $ond = 0;
      $_tit = [];
    ...
    case 'sel_arm_raz':
      ...
      foreach( Dat::_("hol.$ide") as $_dat ){
        ...
        foreach( Dat::_('hol.sel_arm_cel') as $lis_pos ){
          $Sel = Dat::_('hol.sel',$sel); $htm .= "
          <li>
            ".Doc_Val::ima($dat_esq,"sel",$Sel,['class'=>"mar_der-2"])."
            <p>
              <n>{$lis_pos->ide}</n><c>.</c> Sello Solar <n>{$Sel->ide}</n><c>:</c>
              <br>".Doc_Val::let($Sel->arm_raz_des)."
            </p>
          </li>";
          $sel += 4;
          if( $sel > 20 ) $sel -= 20;                  
    ...
    case 'sel_arm_cel':
      ...
      foreach( Dat::_("hol.$ide") as $_dat ){ $htm = "
        ...
        <ul class='ite'>";
        foreach( Dat::_('hol.sel_arm_raz') as $cro ){ $Sel = Dat::_('hol.sel',$lis_pos); $htm .= "
          <li>
            ".Doc_Val::ima($dat_esq,"sel",$Sel,['class'=>"mar_der-1"])."
            <p>
              <n>{$cro->ide}</n><c>.</c> Sello Solar <n>{$Sel->ide}</n><c>:</c>
              <br>".Doc_Val::let($Sel->arm_cel_des)."
            </p>
          </li>";
          $lis_pos ++;
        }$htm .= "
          ...
          for( $fam=1; $fam<=5; $fam++ ){ 
            ...
            <li>
              ...
              <p>
                ...
                <br>".Doc_Val::let($Sel->cro_ele_des)."
              </p>
            </li>";
            $sel++;
            if( $sel > 20 ) $sel -= 20;
              ...
              <div class='-ite'>
                ...
                <ul class='ite' est='sel'>
                  <li>
                    ".Doc_Val::ima($dat_esq,"sel",$Sel,['class'=>"mar_der-1"])."
                    <p>
                      <b class='val'>Dentro</b><c>:</c> Sello Solar <n>{$Sel->ide}</n>
                      <br>".Doc_Val::let($Sel->sol_pla_des)."
                    </p>
                  </li>
                  <li>
      ...
      foreach( Dat::_('hol.pla_cen') as $_dat ){
        ...
        <ul class='ite'>";
          foreach( $_fam_sel[$_dat->ide] as $sel ){
            $Sel = Dat::_('hol.sel',$sel);
            $_pla_mer = Dat::_('hol.pla_mer',$Sel->pla_mer);
            $_pla_hem = Dat::_('hol.pla_hem',$Sel->pla_hem);
            $htm .= "
            <li>
              ".Doc_Val::ima($dat_esq,"sel",$Sel,['class'=>"mar_der-1"])."
              <p>
                Sello <n>{$Sel->ide}</n><c>:</c> {$Sel->nom}<c>,</c>
                <br><n>".intval($Sel->pla_hem_cod)."</n><c>°</c> {$_pla_hem->nom}<c>,</c> <n>".intval($Sel->pla_mer_cod)."</n><c>°</c> {$_pla_mer->nom}
              </p>
            </li>";
          }$htm .= "
        </ul>";
    ...
    case 'hum_ele': 
      ...
      $sel_lis = [];
      foreach( Dat::_('hol.sel_cod') as $Sel ){
        $_fam = Dat::_('hol.sel_cro_fam',$Sel->cro_fam);
        $_hum_ded = Dat::_('hol.hum_ded',$_fam->hum_ded);
        $sel_lis []= Obj::atr([ 
          'hum_ded'=>$_hum_ded->nom, 
          'nom'=>"Tribu ".Tex::art_del($Sel->nom)." $Sel->des_col", 
          'ima_nom'=>[ 'htm'=>Doc_Val::ima($dat_esq,"sel",$Sel,['class'=>"mar-1"]) ],
          'des_cod'=>$Sel->des_cod,
          'ima_cod'=>[ 'htm'=>Doc_Val::ima($dat_esq,"sel_cod",$Sel,['class'=>"mar-1"]) ]
        ]);
      }
      ...
      <nav>";
        ...
        foreach( Dat::_('hol.kin') as $Kin ){
          ...
          if( $Kin->nav_cas != $_nav_cas ){
            ...
            ".Doc_Ope::val(['eti'=>'h3','id'=>"_04-0{$Cas->ide}-",'cas'=>$Cas->ide,'htm'=>"Castillo {$Cas->nom}"])."
            <section data-kin_nav_cas='{$Cas->ide}' class='pad_izq-3'>
              <p cas='{$Cas->ide}'>".Doc_Val::let("Corte {$Cas->des_cor}: {$Cas->des_mis}")."</p>
            ";
          }
          // génesis
  ...
  static function lunas_en_movimiento( string $ide, array $var = [], array $ele = [] ) : string {                    
    ...
    case 'psi_lun_tot':
      ...
      foreach( $_lis as $ite => $htm ){
        $_ []= Doc_Val::ima($dat_esq,"psi_lun",$_lun = Dat::_('hol.psi_lun',$ite),['class'=>"mar_der-2"])."
        <p>
          $htm
          <br>".Doc_Val::let($_lun->fec_ran)."
        </p>";
    ...
    case 'lun_arm':
      if( isset($_atr[1]) ){
        ...
        // descripcion
        case 'des': 
          foreach( Dat::_('hol.lun_arm') as $Hep ){
            $_ []= Doc_Val::let("$Hep->nom (")."<c class='col-4-$Hep->ide'>$Hep->des_col</c>".Doc_Val::let("): $Hep->des");
          }
          break;
        // poderes
        case 'pod':
          foreach( Dat::_('hol.lun_arm') as $Hep ){
            $_ []= Doc_Val::let("$Hep->nom: ")."<c class='col-4-$Hep->ide'>$Hep->des_col</c>".Doc_Val::let(", $Hep->des_pod $Hep->des_car");
          }        
          break;            
        }
      }// luna : heptadas lunares
      else{
        $est_ope['atr'] = [ 'ide','nom','des_col','dia','des_pod' ];
        $est_ope['opc'] []= 'cab_ocu';
    ...
    case 'kin_nav_cas':
      ...
      foreach( Dat::_("hol.$ide") as $Cas ){ $_ [] = 

        Doc_Val::ima($dat_esq,$ide,$Cas,['class'=>"mar_der-2"])."

        <p>
          <b class='ide'>Castillo $Cas->des_col $Cas->des_dir ".Tex::art_del($Cas->des_acc)."</b><c>:</c>
          <br>Ondas Encantadas ".Doc_Val::let($Cas->nav_ond)."
        </p>";
      }          
      break;              
    ...
    case 'sel_cro_fam':
      ...
      foreach( Dat::_('hol.hum_ded') as $_ded ){
        ...
        foreach( explode(', ',$_fam->sel) as $Sel ){
          $Sel = Dat::_('hol.sel',$Sel);
          $_hum_ext = Dat::_('hol.hum_ext',$Sel->hum_ext);
          $sel_lis []= Obj::atr([
            'nom'=>"Tribu ".Tex::art_del($Sel->nom)." $Sel->des_col", 
            'ima_nom'=>[ 'htm'=>Doc_Val::ima($dat_esq,"sel",$Sel,['class'=>"mar-1"]) ],
            'des_cod'=>$Sel->des_cod,
            'ima_cod'=>[ 'htm'=>Doc_Val::ima($dat_esq,"sel_cod",$Sel->ord,['class'=>"mar-1"]) ],
  ...
  static function tratado_del_tiempo( string $ide, array $var = [], array $ele = [] ) : string {                    
    ...
    // psi : vinales
    case 'psi_vin':
      $est_ope['atr'] = ['ide','nom','fec_ini','sin','cro'];
      $est_ope['det_des'] = ['des'];
      //$var['lis']['class'] = "anc-100 mar-2";
      $_ = Doc_Dat::lis("hol.psi_vin", $est_ope, $var);
  ...
  static function telektonon( string $ide, array $var = [], array $ele = [] ) : string {                    
        ...
        <div>
          ...
          <ul>
            ...
            foreach( explode(', ',$_cir->sel) as $sel ){ 
              $lis_pos++; 
              $Sel = Dat::_('hol.sel', $sel == 00 ? 20 : $sel);                      
              $htm .= Doc_Val::let( $Sel->des_pod_tel.( $lis_pos == 3 ? " y " : ( $lis_pos == 4 ? "." : ", " ) ) );
            } $htm .= "
            </p></li>
          </ul>
        </div>
    ...
    // psi : vinales
    case 'psi_vin':
      $est_ope['atr'] = ['ide','nom','fec_ini','sin'];
      $est_ope['det_des'] = ['des'];
      $_ = Doc_Dat::lis("hol.psi_vin", $est_ope, $var);
  ...
  static function tutorial_del_sincronario( string $ide, array $var = [], array $ele = [] ){    
    ...
    // heptadas
    case 'lun_arm':
      foreach( Dat::_("{$dat_esq}.{$ide}") as $Hep ){ 
        $_ []= 
        Doc_Val::let("$Hep->nom (")."<b class='col-4-$Hep->ide'>$Hep->des_col</b>".Doc_Val::let("): $Hep->des");
      }
      break;
    // plasmas y descripciones
    case 'rad':
      $_ = Doc_Dat::des('hol','rad',[ 'des_fue', 'des_pod' ]);
      break;
    // plasmas y afirmaciones
    case 'rad-lec':
      $_ = Doc_Dat::des('hol','rad',[ 'pla_lec' ]);
      break;      
    // kines
    case 'kin_ima':
      foreach( $var as $kin ){
        $_ []= Doc_Val::ima('hol.kin',$kin,['class'=>"mar_der-1"]);
      }
      $var = [ 'lis'=>['class'=>"doc_val mar-aut"] ];
      break;
    // tonos
    case 'ton':
