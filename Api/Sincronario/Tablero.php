...
<?php
  ...
  $Doc->Cla["Api/{$Uri->esq}"] []= "Tablero";

  $Operadores = Doc_Dat::$Tab['ope'];
  
  $Tableros = [
    // ciclos del tiempo s-g
    'ciclo'=>[
      'anillo' =>'ani'
    ],
    // Orden Cíclico
  ...
  if( !in_array($est,['kin','psi']) ) $est = "kin";
  ...
  $lis_var = Dat::est($esq,$tab_ide,'lis');
  ...
  $Doc->Cab['ini']['tab_lis'] = [ 
    ...
    'tip'=>"win",
    'nom'=>$ope['nom'],
    'htm'=>Doc_Dat::tab_ope('lis',"hol.{$tab_ide}",$lis_var) 
  ];  
