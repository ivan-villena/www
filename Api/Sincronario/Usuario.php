...
class Usuario extends Usu {
  ...
  public function listar_transito( array $ope = [], ...$opc ) : string {
    ...
    // listado
    $_lis = [];
    foreach( Dat::get("hol-hum_cas_arm") as $Arm ){
      
      $_lis_cic = [];
      
      foreach( Dat::get('sis-usu_cic_ani',[ 'ver'=>"`usu` = '{$this->key}' AND `arm`=$Arm->ide", 'ord'=>"`ide` ASC" ]) as $_cic ){
      ...
      $_lis []= [ 'ite'=>$Arm->nom, 'lis'=>$_lis_cic ];
    }

    // configuro listado
    Ele::cla($ope['dep'],"dis-ocu");
