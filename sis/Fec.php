...
class Fec {
  ...
  // devuelvo dia
  static function dia( mixed $dat ) : string {
    $_fec = ( !is_object($dat) || get_class($dat)=='stdClass' ) ? Fec::val_dec($dat) : $dat;
    return $_fec->format('w');
  }// cuento dias por periodo : mes | año
  static function dia_cue( string $tip, string | object $val ) : mixed {
    ...
    case 'año':
      ...
      for( $i = 1; $i <= 12; $i++ ){
        ...
        $lis[$i] = Fec::dia_cue('mes',"{$_->año}/$i/1");
        $tot += $lis[$i]; 
      }
      if( $_->mes == 1 ){ 
        $num = $_->dia;
