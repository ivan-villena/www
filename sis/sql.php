...
class sis_sql {
  ...
  static function cod( string $ide = '', array $ope = [], string $tip='ver' ) : array {
    ...
    if( $tip=='agr' ){     
      $_['atr'] = [];
      $_['val'] = [];    
      foreach( api_lis::val_ite($ope['val']) as $pos=>$ite ){
        $_['ite'] = [];
        foreach( $ite as $i=>$v  ){
          if( $pos==0 )
            $_['atr'] []= "`{$i}`";
  ...
  static function est( string $ope, $ide='', ...$opc ) : bool | string | array | object {
    ...
    if( $ope == 'val' ){
      ...
      $_ = FALSE;
      foreach( sis_sql::dec("SHOW TABLE STATUS FROM `{$esq}` WHERE `Name` = '{$ide}'") as $v ){
      ...
      if( $ope == 'lis' ){
        ...
        foreach( sis_sql::dec("SHOW TABLE STATUS FROM `{$esq}`{$ver}") as $v ){
  ...
  static function atr( string $est, string $ope='ver', ...$opc ) : array | object | string {
    ...
    elseif( $ope == 'ver' ){
      ...
      foreach( $dat_lis as $i => $atr ){
        ...
        elseif( $var_dat == 'opc' ){
          $var['dat'] = $var_cue = api_obj::val_dec($var_cue);
        }
        // valores
  ...
  static function reg( string $tip, string $ide, mixed $ope=[] ) : array | object | string {
    ...
    $e = sis_sql::cod($ide,$ope, ( $tip == 'cue' ) ? 'ver' : $tip );

    switch( $tip ){
    case 'ver': 
      $_ = sis_sql::dec("SELECT {$e['atr']} FROM {$e['est']}{$e['jun']}{$e['ver']}{$e['ord']}");
      break;
    case 'cue':  
      $_ = sis_sql::dec("SELECT COUNT( {$e['atr']} ) AS `cue` FROM {$e['est']}{$e['ver']}")[0]['cue'];
      break;
    case 'agr': 
