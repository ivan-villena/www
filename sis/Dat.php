...
class Dat {
  ...
  static function var( string $tip, string | array $ide, array $ele=[], ...$opc ) : string {
    ...
    if( $tip == 'atr' ){
      ...
      if( !empty($_atr = Dat::get_est($esq,$est,'atr',$atr)) ) $_var = [ 
        'nom'=>$_atr->nom, 
        'ope'=>$_atr->var 
      ];
    }
    ...
    elseif( $tip != 'val' ){ 
      ...
      $_var = Dat::var_get($tip,$esq,$est,$atr);
    }
    ...
    return "
    <div".Ele::atr(Ele::cla($ele['ite'],"dat_var",'ini')).">
    ...
    ";   
  }// Cargo variables : api.dat_var
  static function var_get( string $esq, string $dat='', string $val='', string $ide='' ) : array {
    $_ = [];
    
    // cargo todas las estructuras del esquema
    if( empty($dat) ){
      if( !isset(self::$Var[$esq]) ){
        self::$Var[$esq] = Dat::get('dat_var',[
          'ver'=>"`esq`='{$esq}'", 'niv'=>['dat','val','ide'], 'ele'=>["atr"], 'red'=>"atr"
        ]);
      }
    }// cargo por agrupacion
    elseif( empty($val) ){
      if( !isset(self::$Var[$esq][$dat]) ){
        self::$Var[$esq][$dat] = Dat::get('dat_var',[
          'ver'=>"`esq`='{$esq}' AND `dat`='{$dat}'", 'niv'=>['val','ide'], 'ele'=>["atr"], 'red'=>"atr"
        ]);
      }
    }// cargo uno
    else{
      if( !isset(self::$Var[$esq][$dat][$val]) ){
        self::$Var[$esq][$dat][$val] = Dat::get('dat_var',[
    ...
    if( !empty($ide) ){
      $_ = isset(self::$Var[$esq][$dat][$val][$ide]) ? self::$Var[$esq][$dat][$val][$ide] : [];
    }
    elseif( !empty($val) ){
      $_ = isset(self::$Var[$esq][$dat][$val]) ? self::$Var[$esq][$dat][$val] : [];
    }
    elseif( !empty($dat) ){
      $_ = isset(self::$Var[$esq][$dat]) ? self::$Var[$esq][$dat] : [];
    }
    else{
      $_ = isset(self::$Var[$esq]) ? self::$Var[$esq] : [];
    }
  ...
  static function val_sum(  string $dat, mixed $val = [], array $ope = [] ) : string {
    $_ = "";
    extract( Dat::ide($dat) );
    $_ide = self::$IDE."val_sum-$esq-$est";
    // estructuras por esquema
    foreach( Dat::var_get($esq,'ope','sum') as $ide => $ite ){
  
      $_ .= Dat::var($esq,"ope.sum.$ide",[
        'ope'=>[ 'id'=>"{$_ide} sum-{$ide}" ],
  ...
  static function val_ver( array $ope, string $ide, string $eje, ...$opc ) : string {
    ...
    $ope_ver_var = function ( array $var = [], array $ele = [], ...$opc ) : string {
      ...
      foreach( ['ini','fin'] as $ide ){
        ...
        if( isset($var[$ide]) ) $_ .= Dat::var('dat',"ope.ver.$ide", [ 'ope'=>$_ite($ide,$var) ]);
      }

      // imprimo cada
      if( isset($var['inc']) ){
        $_ .= Dat::var('dat',"ope.ver.inc", [ 'ope'=>$_ite('inc',$var) ]);
      }
      ...
      if( isset($var['lim']) ){
        ...
        <div class='doc_ren tam-ini'>
          ...
          ".Dat::var('dat',"ope.ver.lim", [ 'ope'=>$_ite('lim',$var) ] )."
  ...
  static function lis( string | array $dat, array $ope = [], array $ele = [] ) : string {
    ...
    if( isset($ope['ope']) ){
      ...
      if( !empty( $_ope = Obj::val_nom(self::$Lis['ope'],'ver',Lis::val_ite($ope['ope'])) ) ){
        ...
        $_ .= Doc::nav('bot', $_ope,[ 'lis'=>['class'=>"mar_hor-1"] ],'ico','tog');
      }    
    }
    /////////////////////////////////////////
    // 3- imprimo listado ///////////////////
        ...
        if( isset($ope['val']['acu']) ){
          ...
          <form class='ide-acu'>
            <fieldset class='doc_inf doc_ren'>
              ...
              ".Dat::var('dat',"ope.ver.tot", [ 'ope'=>[ 'id'=>"{$_ide}-tot" ] ])."
              
              ".Dat::var('dat',"ope.ver.tod", [ 'ope'=>[ 'id'=>"{$_ide}-tod", 'onchange'=>"{$eje_val}('tod',this);" ] ])."
          ...
          <form class='ide-$tip doc_ren jus-ini mar_izq-2'>
            ...
            foreach( $est_ope['atr'] as $atr ){
              ...
              $htm .= Dat::var('val',$atr,[
                ...
                'ope'=>[ 'tip'=>'num_bin', 'id'=>"{$_ide} _{$esq}-{$est}-{$atr}", 
              ...
              if( !empty($est_ope["{$pre}_{$ide}"]) ){ $htm = "
                ...
                foreach( $est_ope["{$pre}_{$ide}"] as $atr ){
                  $htm .= Dat::var('val',$atr,[ 
                    ...
                    'ope'=>[ 'tip'=>'num_bin', 'id'=>"{$_ide}-{$atr}-{$ide}", 'onchange'=>"{$_eje}_tog(this);" ] 
                  ]);
                }$htm .= "
                </form>";
                $lis_dep[] = [ 
                  'ite'=> Dat::var_get('dat','lis','ver',$ide)['nom'], 
                  'htm'=> $htm
                ];
              }
            }
  ...
  static array $Tab = [
    ...
    'ope' => [
      ...
      'opc' => [ 'ide'=>"opc", 'ico'=>"num_bin", 'nom'=>"Opciones", 'des'=>"" ],
  ...
  static function tab_ope( string $tip, string $dat, array $ope = [], array $ele = [] ) : string {
    ...
    case 'val':
      ...
      <form class='ide-acu'>
        <fieldset class='doc_inf doc_ren'>
          ...
          $_ .= Dat::var('dat',"ope.ver.tot", [ 'ope'=>[ 'id'=>"{$_ide}-tot" ] ]);
      ...
      $_ .= "
      <section class='ide-cue doc_inf pad_hor-2'>
    ...
    case 'opc':
      ...
      $_opc_var = function( $_ide, $tip, $esq, $ope, ...$opc ) : string {
        ...
        $ope_var = Dat::var_get($esq,'tab',$tip);
      ...
      if( !empty($ope[$tip_opc = 'sec']) ){
        ...
        <form".Ele::atr($ele_ope).">
          <fieldset class='doc_inf doc_ren'>
            ...
            // operadores globales
            if( !empty($tab_sec = Dat::var_get('dat','tab',$tip_opc)) ){ $_ .= "
              <div class='doc_val'>";
              foreach( Dat::var_get('dat','tab',$tip_opc) as $ide => $ite ){
                if( isset($ope[$tip_opc][$ide]) ){ 
                  $_ .= Dat::var('dat',"tab.$tip_opc.$ide", [ 
                    'val'=>$ope[$tip_opc][$ide], 
                ...
                <div class='doc_ren'>";
                  ...
                  if( isset($ope['val']['acu']) ){ 
                    foreach( array_keys($ope['val']['acu']) as $ite ){
                      $_ .= Dat::var('dat',"tab.$tip_opc.{$ide}_{$ite}", [
      ...
      if( !empty($opc) ){
        ...
        foreach( $opc as $atr ){  
          $htm = "";
          foreach( Dat::var_get($esq,'tab',"{$tip}-{$atr}") as $ide => $val ){
            $val_ope = [
            ...
            $htm .= Dat::var($esq,"tab.{$tip}-{$atr}.$ide", $val_ope);
          }          
          // busco datos del operador 
          if( !empty($htm) && !empty($_ope = Dat::var_get($esq,'tab',$tip,$atr)) ){
            $ele_ope = $ele['ope'];
