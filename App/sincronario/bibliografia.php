...
function listado( string $lib, string $ide, array $ope = [] ){
  ...
  case 'tut':
    ...
    case 'rad':
      ...
      $_ = Doc_Dat::atr_des($dat_esq,'rad',[ 'des_fue', 'des_pod' ]);
      break;
    // plasmas y afirmaciones
    case 'rad-lec':

      $_ = Doc_Dat::atr_des($dat_esq,'rad',[ 'pla_lec' ]);
      break;      
    // kines
    case 'kin_ima':
  ...
  case 'fac':
    ...
    // kin : katun del kin
    case 'kin':
      $_kin = Dat::_('hol.kin',$ope['ide']);
      $_sel = Dat::_('hol.sel',$_kin->arm_tra_dia);
      $_pol = Dat::_('hol.uni_flu_res',$_sel->flu_res);
      $_pla = Dat::_('hol.uni_sol_pla',$_sel->sol_pla);
    ...
    case 'kin_fec':
      ...
      <table>";
        if( !empty($ope['tit']) ){ $_.="
          <caption>".( !empty($ope['tit']['htm']) ? "<p class='tit'>".Doc_Val::let($ope['tit']['htm'])."</p>" : '' )."</caption>";
        }$_.="
  ...
  case 'enc': 
    ...
    case 'ton':
      ...
      $_ = Doc_Dat::lis("hol.ton", $est_ope, $ope );
      break;
    ...
    case 'ton_ond':
      ...
      $_ = Doc_Dat::lis($_dat,[ 'atr_dat'=>$_atr, 'tit'=>$_tit, 'opc'=>['cab_ocu'] ], $ope);              
      break;
    ...
    case 'sel_cro_fam':
      ...
      foreach( Dat::_('hol.uni_pla_cen') as $_pla ){
        $_hum = Dat::_('hol.uni_hum_cen',$_pla->ide);
        $_fam = Dat::_('hol.'.$ide,$_pla->fam);
        $htm = 
        Doc_Val::ima($dat_esq,"uni_pla_cen",$_pla,['class'=>"mar_der-2",'style'=>"min-width: 9rem; height:7rem;"])."
        <div>
          <p><b class='ide'>Kin {$_fam->nom}</b><c>:</c> {$_fam->des_rol}</p>
          <div class='-val fic mar-2'>
            ".Doc_Val::ima($dat_esq,"uni_hum_cen",$_hum)."
            <c class='sep'>=></c>
            <c class='_lis ini'>{</c>";
      ...
      foreach( Dat::_('hol.'.$ide) as $_dat ){
        if( $val_sel || in_array($_dat->ide,$val) ){ 
          ...
          for( $sol_pla=1; $sol_pla<=2; $sol_pla++ ){
            $_pla = Dat::_('hol.uni_sol_pla',$pla);
            $_sel = Dat::_('hol.sel',$sel);
            $_par = Dat::_('hol.sel',$_sel->par_ana); 
            if( $orb != $_pla->orb ){
              $_orb = Dat::_('hol.uni_sol_orb',$orb = $_pla->orb); $htm .= "
            ...
            <li>
              ...
              <div class='-ite'>
                ...
                ".Doc_Val::ima($dat_esq,"uni_sol_pla",$_pla,['class'=>"mar_der-1"])."
    ...
    case 'uni_pla_pos':
      ...
      foreach( Dat::_('hol.uni_pla_cen') as $_dat ){
        $_fam = Dat::_('hol.sel_cro_fam',$_dat->fam);
        $htm = "
        <div class='tex_ali-cen'>
          <p>Kin <b class='ide'>{$_fam->nom}</b></p>
          ".Doc_Val::ima($dat_esq,"uni_pla_cen",$_dat,['class'=>"mar_der-1",'style'=>"min-width: 17rem; height: 11rem;"])."
        </div>
        <ul class='ite'>";
          foreach( $_fam_sel[$_dat->ide] as $sel ){
            $_sel = Dat::_('hol.sel',$sel);
            $_pla_mer = Dat::_('hol.uni_pla_mer',$_sel->pla_mer);
            $_pla_hem = Dat::_('hol.uni_pla_hem',$_sel->pla_hem);
            $htm .= "
            <li>
              ".Doc_Val::ima($dat_esq,"sel",$_sel,['class'=>"mar_der-1"])."
              <p>
    ...
    case 'uni_hum_ele': 
      $ele_tit = [];
      $col = 4;
      foreach( Dat::_('hol.uni_hum_ext') as $_ext ){
        $_ele = Dat::_('hol.sel_cro_ele',$_ext->ele); 
        $nom = explode(' ',Tex::art_del($_ele->nom)); $cla = array_pop($nom); $nom = implode(' ',$nom);
        $ele_tit[$lis_pos] = [ 
          'eti'=>"div", 'class'=>"ite", 'htm'=> Doc_Val::ima($dat_esq,"uni_hum_ext",$_ext,['class'=>"mar_der-1"])."                  
      ...
      foreach( Dat::_('hol.sel_cod') as $_sel ){
        ...
        $_hum_ded = Dat::_('hol.uni_hum_ded',$_fam->hum_ded);
        $sel_lis []= Obj::atr([ 
    ...
    case 'uni_hum_ext':
      ...
      foreach( Dat::_('hol.'.$ide) as $_dat ){
        $_ele = Dat::_('hol.sel_cro_ele',$_dat->ele); $_ []= "
          ...
          ".Doc_Val::ima($dat_esq,"uni_hum_ext",$_dat,['class'=>"mar_der-1"])."
    ...
    case 'uni_hum_ded':
      ...
      foreach( Dat::_('hol.'.$ide) as $_dat ){
        $_fam = Dat::_('hol.sel_cro_fam',$_dat->fam); $_ []= "
          ...
          ".Doc_Val::ima($dat_esq,"uni_hum_ded",$_dat,['class'=>"mar_der-1"])."
      ...
      <form class='ope_inf' data-esq='hol' data-est='$ide'>
        ...
        <div class='-val'>
          ...
          <fieldset class='app_ope'>
            ...
            ".Doc_Val::ico('lis_fin',[ 'eti'=>"button", 'title'=>"Ir al Kin...", 'onclick'=>"{$_eje},'nav',this);" ])."
          </fieldset>
  ...
  case 'lun': 
    ...
    case 'lun_arm_col':
      ...
      $est_ope['opc'] []= 'cab_ocu';
      $_ = Doc_Dat::lis("hol.lun_arm", $est_ope, $ope );
      break;
    ...
    case 'sel_cro_fam':
      ...
      foreach( Dat::_('hol.uni_hum_ded') as $_ded ){
        $_fam = Dat::_('hol.sel_cro_fam',$_ded->fam);
        $fam_tit[$lis_pos] = [
          'eti'=>"div", 
          'class'=>"-ite", 
          'htm'=> Doc_Val::ima($dat_esq,"uni_hum_ded",$_ded,['class'=>"mar_der-1"])."                  
        ...
        foreach( explode(', ',$_fam->sel) as $_sel ){
          ...
          $_hum_ext = Dat::_('hol.uni_hum_ext',$_sel->hum_ext);
          $sel_lis []= Obj::atr([
            'nom'=>"Tribu ".Tex::art_del($_sel->nom)." $_sel->des_col", 
            'ima_nom'=>[ 'htm'=>Doc_Val::ima($dat_esq,"sel",$_sel,['class'=>"mar-1"]) ],
            'des_cod'=>$_sel->des_cod,
  ...
  case 'tie':
    ...
    case 'psi_vin':
      ...
      $_ = Doc_Dat::lis("hol.psi_ani_vin", $est_ope, $ope);
  ...
  case 'tel':
    ...
    case 'uni_sol_cir':
      ...
      foreach( Dat::_('hol.'.$ide) as $_cir ){
        ...
        $pla_ini = Dat::_('hol.uni_sol_pla',$pla[0]);
        $pla_fin = Dat::_('hol.uni_sol_pla',$pla[1]);
        $htm = 
  ...
  case 'rin':
    ...
    case 'psi_lun_dia':
      ...
      elseif( empty($est_ope['atr']) ){
        $est_ope['atr'] = [];
        $_ = Doc_Dat::lis("hol.lun", $est_ope, $ope );
      }
      break;
    ...
    case 'rad_pla':
      ...
      case 'cub':
        ...
        foreach( Dat::_('hol.'.$dat_ide) as $rad ){
          ...
          $cha = Dat::_('hol.uni_hum_cha', $rad->hum_cha);
          $_ []= 
          "<div>".
            Doc_Val::ima($dat_esq,'uni_hum_cha',$cha,['class'=>"mar_der-2"]).
  ...
  return is_array($_) ? Doc_Dat::est( $_, "hol.$ide", $ope ) : $_;
}