<?php
  ...
  function hep_est_dia(){
    ...
    foreach( Dat::_('hol.psi_est_dia') as $_dia ){
      ...
      $_ .= "
      UPDATE `hol-psi_hep_est_dia` SET
        `ton` = $ton
      WHERE 
        `ide` = $_dia->ide;<br>";
    }
  ...
  function hep_pla(){
    $_ = "";
    $psi_pos = 1;
    foreach( Dat::_('hol.psi_hep_pla') as $_hep ){
      $_ton = Dat::_('hol.ton',$_hep->ton);      
      $_lun = Dat::_('hol.psi_ani_lun',$_ton->ide);
      $_est = Dat::_('hol.psi_hep_est',$_hep->arm);
      ...
      $psi_pos += 7;
      $_ .= "UPDATE `hol-psi_hep_pla` SET `nom`='$nom', `des`='$des', `est`=$est, `lun`=$lun, `psi`='$psi' WHERE  `ide`=$_hep->ide;<br>";
    }
    return $_;
  }