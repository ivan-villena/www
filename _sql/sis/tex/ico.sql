-- Active: 1670107173962@@127.0.0.1@3306@c1461857_api
--
-- estilo 
  DELETE FROM `sis-tex_ico_gru`
  ;
  INSERT INTO `sis-tex_ico_gru` VALUES
    (NULL,'material-icons',					 'https://fonts.googleapis.com/css?family=Material+Icons'),
    (NULL,'material-icons-outlined', 'https://fonts.googleapis.com/css?family=Material+Icons+Outlined'),
    (NULL,'material-icons-two-tone', 'https://fonts.googleapis.com/css?family=Material+Icons+Two+Tone'),
    (NULL,'material-icons-round',		 'https://fonts.googleapis.com/css?family=Material+Icons+Round'),
    (NULL,'material-icons-sharp',		 'https://fonts.googleapis.com/css?family=Material+Icons+Sharp')
  ;
--
-- icono 
  DELETE FROM `sis-tex_ico`
  ;
  -- 
  -- Sistema
    DELETE FROM `sis-tex_ico` WHERE `ide` LIKE 'app%';
    INSERT INTO `sis-tex_ico` VALUES  
      ('app',         'home'),      
      ('app_ses',	    'manage_accounts'),
      ('app_ses-ini',	'login'),
      ('app_ses-fin',	'logout'),
      ('app_cab',			'menu'),
      ('app_nav',	    'menu_open'),
      ('app_art',     'article'),
      ('app_val',     'document_scanner')
    ;    
    DELETE FROM `sis-tex_ico` WHERE `ide` LIKE 'usu%';
    INSERT INTO `sis-tex_ico` VALUES
      ('usu',	    'person'),      
      ('usu_pas',	'password'),
      ('usu_ubi',	'share_location'),
      ('usu_mai',	'email'),
      ('usu_tel',	'phone'),
      ('usu_web',	'web_asset'),
      ('usu_gru',	'group'),
      ('usu_age',	'menu_book')
    ;
    DELETE FROM `sis-tex_ico` WHERE `ide` LIKE 'dat%';
    INSERT INTO `sis-tex_ico` VALUES

      ('dat',			'database'),
      -- atributos
      ('dat_pos',	''),
      ('dat_ide',	''),
      ('dat_nom',	''),
      ('dat_des',	'help_center'),
      ('dat_tip',	'input'),
      ('dat_var',	'variables'),
      ('dat_err',	'gpp_bad'),
      ('dat_adv',	'gpp_maybe'),
      -- tipos
      ('dat_ope', 'play_arrow'),      
      ('dat_tab',  'dataset'),
      ('dat_lis',	 'table_view')
    ;
    DELETE FROM `sis-tex_ico` WHERE `ide` LIKE 'doc%';
    INSERT INTO `sis-tex_ico` VALUES
      ('doc',	''),
      ('doc_lib',	'auto_stories'),
      ('doc_inf',	'description')
    ;
    DELETE FROM `sis-tex_ico` WHERE `ide` LIKE 'val%';
    INSERT INTO `sis-tex_ico` VALUES
      ('val',	    'settings'),
      -- operaciones de abm
      ('val-ver',	'search'),
      ('val-tod',	'checklist'),
      ('val-nad',	'flaky'),
      ('val-act',	'sync'),
      ('val-agr',	'add'),
      ('val-mod',	'edit'),
      ('val-eli',	'delete'),
      ('val-ini',	'done'),
      ('val-fin',	'close'),
      -- togs : item - expandir - contraer
      ('val_tog',	    'arrow_drop_down'),
      ('val_tog-tod',	'expand_circle_down'),
      ('val_tog-nad',	'expand_circle_down'),
      -- filtros : mostrar - ocultar
      ('val_ver',	    ''),
      ('val_ver-tod',	'visibility'),
      ('val_ver-nad',	'visibility_off'),
      -- seleccion : todo - nada
      ('val_sel',	''),
      ('val_sel-tod',	'select_all'),
      ('val_sel-nad',	'deselect'),
      -- estado : activo - inactivo
      ('val_est',	''),
      ('val_est-tod',	'toggle_on'),
      ('val_est-nad',	'toggle_off'),
      -- movimiento
      ('val_mov',     'open_with'),  
      ('val_mov-hor', 'swap_horiz'),
      ('val_mov-ver', 'import_export'),
      ('val_mov-arr', 'north'),
      ('val_mov-aba', 'south'),
      ('val_mov-izq', 'west'),
      ('val_mov-der', 'east'),
      ('val_mov-dyr', 'north_east'),
      ('val_mov-dyb', 'north_west'),
      ('val_mov-iyr', 'south_east'),
      ('val_mov-iyb', 'south_west')
    ;    
  --
  -- Api
    DELETE FROM `sis-tex_ico` WHERE `ide` LIKE 'lis%';
    INSERT INTO `sis-tex_ico` VALUES

      ('lis',			'list_alt'),
      -- tipos
      ('lis_val',	 'list'),      
      ('lis_ite',	 'view_list'),
      ('lis_opc',  'checklist_rtl'),
      -- navegacion      
      ('lis_ini',	'first_page'),
      ('lis_fin',	'last_page'),
      ('lis_pre',	'navigate_before'),
      ('lis_pos',	'navigate_next'),      
      -- operaciones
      ('lis_tod',	'playlist_add_check'),
      ('lis_nad',	'playlist_remove'),
      ('lis_cue',	'summarize'),  
      ('lis_ver',	'filter_alt'),
      ('lis_jun',	'join_inner'),
      ('lis_gru',	'low_priority'),
      ('lis_ord',	'sort'),
      ('lis_lim',	'segment'),
      ('lis_mov',	'unfold_more')
    ;
    DELETE FROM `sis-tex_ico` WHERE `ide` LIKE 'opc%';
    INSERT INTO `sis-tex_ico` VALUES

      ('opc',	    ''),
      ('opc_bin',	'rule'),
      ('opc_uni',	'radio_button_checked'),
      ('opc_mul',	'check_box')
    ;    
    DELETE FROM `sis-tex_ico` WHERE `ide` LIKE 'num%';
    INSERT INTO `sis-tex_ico` VALUES

      ('num',	'tag'),
      -- valores
      ('num_val',	'pin'),      
      ('num_sep',	''),
      ('num_sup',	'superscript'),
      ('num_inf',	'subscript'),      
      -- tipos
      ('num_bit',	'1k'),
      ('num_int',	'numbers'),  
      ('num_dec',	'percent'),
      ('num_ran',	'linear_scale'),
      -- operadores      
      ('num_ope',	'calculate'),      
      ('num_sum',	'add'),
      ('num_res',	'remove'),
      ('num_mul',	'close'),
      ('num_div',	'percent'),
      -- calculos
      ('num_fun',	'function'),
      ('num_adi',	'functions')
    ;
    DELETE FROM `sis-tex_ico` WHERE `ide` LIKE 'tex%';
    INSERT INTO `sis-tex_ico` VALUES    

      ('tex',			'edit_note'),
      -- tipos
      ('tex_val',	'rtt'),
      ('tex_let',	'title'),
      ('tex_pal',	'text_fields'),
      ('tex_ora',	'short_text'),
      ('tex_par',	'chat'),
      -- operadores
      ('tex_ord',	'sort_by_alpha'),
      ('tex_mov',	'wrap_text'),
      ('tex_rot',	'text_rotation_down'),
      -- carteles :
      ('tex-ver',	'comment'),
      ('tex-err',	'error'),
      ('tex-adv',	'warning'),
      ('tex-opc',	'help'),
      ('tex-val',	'check_circle')          
    ;
    DELETE FROM `sis-tex_ico` WHERE `ide` LIKE 'fec%';
    INSERT INTO `sis-tex_ico` VALUES

      ('fec',			'calendar_today'),      

      -- tipos
      ('fec_tie',	'access_time'),
      ('fec_dia',	'event'),
      ('fec_sem',	'date_range'),
      ('fec_mes',	'calendar_month'),
      ('fec_año',	'event_note'), 

      -- operaciones
      ('fec_val', 'edit_calendar')

    ;
    DELETE FROM `sis-tex_ico` WHERE `ide` LIKE 'fig%';
    INSERT INTO `sis-tex_ico` VALUES
      ('fig',	'auto_graph'),      
      -- valores
      ('fig_col',	'palette'),
      ('fig_ima',	'image'),
      -- tipos
      ('fig_geo',	'shape_line'),
      ('fig_pun',	'scatter_plot'),
      ('fig_lin',	'trending_down'),
      ('fig_pol',	'polyline')
    ;
    DELETE FROM `sis-tex_ico` WHERE `ide` LIKE 'arc%';
    INSERT INTO `sis-tex_ico` VALUES  

      ('arc',			'attach_file'),
      -- tipos
      ('arc_val',	'note'),
      ('arc_fic',	'folder'),
      ('arc_url',	'language'),
      ('arc_tex',	'text_snippet'),
      ('arc_ima',	'collections'),
      ('arc_mus',	'audio_file'),
      ('arc_vid',	'video_file'),
      ('arc_eje',	'play_circle')

    ;
    DELETE FROM `sis-tex_ico` WHERE `ide` LIKE 'eje%';
    INSERT INTO `sis-tex_ico` VALUES

      ('eje',	    'terminal')
    ;
    DELETE FROM `sis-tex_ico` WHERE `ide` LIKE 'obj%';
    INSERT INTO `sis-tex_ico` VALUES

      ('obj',	'widgets'),
      -- tipos      
      ('obj_pos',	'format_list_numbered'),
      ('obj_nom',	'data_array'),
      ('obj_atr',	'data_object')
    ;
    DELETE FROM `sis-tex_ico` WHERE `ide` LIKE 'ele%';
    INSERT INTO `sis-tex_ico` VALUES
      ('ele', 'code'),
      -- valores
      ('ele_htm', 'html'),
      ('ele_css', 'css'),
      ('ele_eje', 'javascript')
    ;
  --
--