...
class Fec {
  ...
  static function _( string $ide, $val = NULL ) : string | array | object {
    ...
    $_ = $_dat = Dat::get_est('fec',$ide,'dat');
  ...
  static function var( string $tip, mixed $dat = NULL, array $ope = [], ...$opc ) : string {
    ...
    case 'val':
      $ope['eti'] = "time";
      $ope['htm'] = Tex::let(Fec::val_var($dat));
      Ele::cla($ope,"fec",'ini');
      $ope['value'] = $dat;
      $_ = Ele::eti($ope);
      break;
  ...
  static function año_ran( int $ini, int $fin ) : string {
    ...
    if( $ini < 0 && $fin < 0  ){
      ...
      $_ = Num::val_int( $ini * - 1 )." - ".Num::val_int( $fin * - 1). " A.C.";
    }
    elseif( $ini > 0 && $fin > 0 ){

      $_ = Num::val_int( $ini )." - ".Num::val_int( $fin ). " D.C.";
    }
    else{
      $_ = Num::val_int( $ini * - 1 )." A.C. - ".Num::val_int( $fin ). " D.C.";
    }
