...
class api_app {
  ...
  static tip( $val ){
    ...
    // busco
    return !!$api_app._tip[$ide] ? $api_app._tip[$ide] : false;
  }  

  /* -- Estructura -- */
  static est( $esq, $est, $ope, $dat ) {

    let $={}, $_ = $api_app._est[$esq][$est];
    ...
    if( $ope ){
      ...
      if( $_ && $dat ){
        switch( $.ope_atr[0] ){
        case 'val':
          $_ = api_obj.val( api_app.dat($esq,$est,$dat), $_ );
          break;
        }
      }
    }
  ...
  static est_atr_dat( $esq, $est, $atr ){
    ...
    if( $api_app._est[$esq][$est]?.atr[$atr] && ( $.dat_atr = $api_app._est[$esq][$est]?.atr[$atr].var.dat ) ){
