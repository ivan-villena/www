<?php
  ...
  class _app {
    ...
    // Valores : absoluto o con dependencias ( _api.dat->est ) 
  ...
  class _app_dat {
    ...
    static function opc( string $ide, mixed $dat, array $ope = [], ...$opc ) : string {
      ...
      $ele_val = [ 
        'eti'=>[ 'name'=>"val", 'title'=>"Seleccionar el valor buscado...", 
      ...
      if( $opc_esq || $opc_est ){
        ...
        if( $opc_esq ){
          $dat_esq = [];
          $ele_esq = [ 
            'eti'=>[ 'name'=>"esq", 'title'=>"Seleccionar el Esquema de Datos...", 
              'style'=>$opc_ope_tam, 'onchange'=>$_eje.",'esq');" 
            ] 
          ];
        }// operador por estructuras
        $ele_est = [ 
          'eti'=>[ 'name'=>"est", 'title'=>"Seleccionar la Estructura de datos...", 
    ...
    // Tabla por Estructura ( con html y sin cabecera )
    ...
    static function fic_atr( mixed $dat, array $ope = [], ...$opc ) : string {
      $_ide = self::$IDE."atr";
      $_eje = self::$EJE."atr";
      $_ = "";
  ...
  class _app_est {
    ...
    static function ope( string $tip, string | array $dat, array $ope = [], array $ele = [], ...$opc ) : string {
      ...
      case 'cue':
        ...
        "._doc_lis::val( _app_val::cue('dat', $ope['est'], [ 'ide'=>$_ide ]), [
            'dep'=>[ ], 'opc'=>[ 'tog', 'ver', 'cue' ] 
          ] 
        );
        break;
      ...
      if( isset($ope['est']) ){
        ...
        foreach( $ope_est as $esq => $est_lis ){
          ...
          foreach( $est_lis as $est ){
            ...
            $_ .= _app_est::atr("{$esq}.{$est}",$ope,$ele,...$opc);
          }
        }
      }
  ...
  class _app_tab {
    ...
    static array $OPE = [
      ...
      'val' => [ 'ide'=>"val", 'ico'=>"dat_tip", 'nom'=>"Elementos del Tablero",
        'des'=>"" 
      ],
    ...
    static function ope( string $tip, string $dat, array $ope = [], array $ele = [], ...$opc ) : string {
          ...
          <section ide = 'ver'>
            ...
            <form ide = 'dat'>
              <fieldset class = 'inf ren'>
                ...
                "._app_val::ver('dat', $ope['est'], [ 
                  'ope'=>[ 'onchange'=>"{$_eje}_ver('dat',this);" ] 
                ], 'ope_tam')."
      ...
      case 'lis': 
        ...
        foreach( $_ope as $ope_ide => &$ope_lis ){
          ...
          $ope_lis['htm'] = _app_est::ope($ope_ide,$dat,$lis_ope,$ele,...$opc);
        }
        $_ =
          _doc_val::nav('bar',$_ope,[ 'lis'=>[] ],'tog')."
          
          <div var='est' style='height: 70vh;'>
            "._app_est::lis($dat,$lis_ope,$ele,...$opc)."
          </div>";
        break;
      }
      return $_;
