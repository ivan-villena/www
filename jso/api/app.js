...
class _app {
  ...
  ini = ( $ = {} ) => {
    ...
    if( this.sec ){
      // por menu
      if( $_api.app_uri && $_api.app_uri.cab ){
        ...
        if( $_api.app_uri.cab == 'tab' ){
          ...
          // muestro panel
          _app_ope,tog('nav','tab');
        } 
        // indice por artículo
        else if( $_api.app_uri.art && ( $.art_nav = this.nav.querySelector('nav[ide="art"] ul.lis.nav') ) ){          
          // muestro panel
