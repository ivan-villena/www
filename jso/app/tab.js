...
class app_tab {
  ...
  static ini(){
    ...
    ['sec','pos'].forEach( $ope => {
      ...
      if( $_app.tab.opc[$ope] ){
        $_app.tab.opc[$ope].querySelectorAll(
          `form[class*="ide-"] [onchange*="app_tab."]:is( input:checked, select:not([value=""]) )`
        ).forEach( 
          $inp => app_tab[`opc_${$ope}`]( $inp )
        );
      }
    });
    ...
    $_app.ope_val.acu.forEach( 
      $ite => ( $.ele = $_app.tab.val.acu.querySelector(`[name="${$ite}"]:checked`) ) && app_tab.val_acu($.ele) 
    );
  ...
  static act( $dat ){
    ...
    $dat = !$dat ? $_app.ope_val.acu : api_lis.ite($dat);
    ...
    if( $_app.tab.val.acu ){ 
      ...
      app_val.acu($_app.tab.lis, $_app.tab.val.acu, ...$dat);
      ...
      if( $_app.tab.val.sum ){
        ...
        app_val.sum($.tot, $_app.tab.val.sum);
      }

      // listado asociado:

      // -> actualizo acumulados
      if( !$_app.est.val.acu.querySelector(`[name="tod"]:checked`) ) app_est.val_acu();

      // -> ejecuto filtros + actualizo totales
      if( $_app.est.ver ) app_est.ver();
    }
    ...
    if( ( $_app.tab.opc.pos ) && ( $.ima = $_app.tab.opc.pos.querySelector(`[name="ima"]`) ) ){
      ...
      if( $.ope.length > 0 ) app_tab.opc_pos($.ima);
    }

    // actualizo cuentas
    if( $_app.tab.val.cue ){

      app_val.cue('act', $_app.tab.lis.querySelectorAll(`${$_app.tab.cla}[class*=_val-]:is([class*=_bor],[class*=_act])`), $_app.tab.val.cue );
    }
  }
  ...
  static ver( $tip ){
    ...
    app_val.ver($tip, api_lis.val($_app.tab.lis.querySelectorAll(`${$_app.tab.cla}`)), $_app.tab.ver );

    // marco seleccionados
    api_ele.act('cla_eli',$_app.tab.lis.querySelectorAll('._val-ver_bor'),'_val-ver_bor');
    if( $_app.tab.val.acu && $_app.tab.val.acu.querySelector(`[name="ver"]:checked`) ){
      api_ele.act('cla_agr',$_app.tab.lis.querySelectorAll(`._val-ver-`),'_val-ver_bor');
    }
    ...
    case 'ver':
      for( const $ide in $_app.ope_val.ver ){
        ...
        if( $.ele = $_app.tab.ver.querySelector(`${$_app.ope_val.ver[$ide]}:not([value=""])`) ){
          ...
          app_tab.ver($ide,$.ele,$_app.tab.lis);
  ...
  static val_acu( $dat, $ope ){
    ...
    if( !$.var_ide && $ope ) $ = app.var( $dat = $_app.tab.val.acu.querySelector(`[name="${$ope}"]`) );
    ...
    if( $.var_ide == 'opc' ){
      ...
      api_ele.act('cla_eli',$_app.tab.lis.querySelectorAll(`.${$.cla_ide}_bor`),`${$.cla_ide}_bor`);
      if( $dat.checked ) api_ele.act('cla_agr',$_app.tab.lis.querySelectorAll(`.${$.cla_ide}-`),`${$.cla_ide}_bor`);
    }
    // actualizo calculos + vistas( fichas + items )        
    app_tab.act($.var_ide);
  ...
  static opc_pos( $dat ){
    ...
    case 'col':
      ...
      $_app.tab.lis.querySelectorAll($.eli).forEach( $e => api_ele.cla($e,$.ope,'eli','ini' ) );

      if( $dat.value ){

        $ = api_dat.ide($dat.value,$);

        $.col = api_dat.opc('col', ...( ( $.dat = $dat.options[$dat.selectedIndex].getAttribute('dat') ) ? $.dat : $dat.value ).split('.') );

        $.col = ( $.col && $.col.val ) ? $.col.val : 0;

        $_app.tab.lis.querySelectorAll($.agr).forEach( $e =>{

          if( $._dat = api.dat($.esq,$.est,$e.dataset[`${$.esq}_${$.est}`]) ){

            $.val = ( $.col == 1 && $._dat[$.atr] > $.col ) ?  0 : $._dat[$.atr];

            $e.classList.add(`${$.ope}${$.col}-${ $.val === 0 ? $.val : api_num.ran($.val,$.col) }`);
          }
        });
      }
      break;          
    ...
    case 'ima':
      ...
      if( $dat.value ){
        ...
        $.fic = api_dat.opc('ima', ...( ( $.dat = $dat.options[$dat.selectedIndex].getAttribute('dat') ) ? $.dat : $dat.value ).split('.') );
        ...
        $_app.tab.lis.querySelectorAll($_app.tab.cla).forEach( $e => {
          ...
          if( $.ima.pos || $.ima.mar || $.ima.ver || $.ima.opc ){
            ...
            if( $.ima.pos && $e.classList.contains('_val-pos-') ){ 
              ...
              $e.classList.forEach( $cla_nom => {
                if( /_val-opc-/.test($cla_nom) ) return $.htm = api_dat.ima($e,$);
              });
            }
          }// todos
          else{
          ...
          if( $.htm ){
            ( $.ima_ele = $e.querySelector('.ima') ) ? api_ele.mod($.htm,$.ima_ele) : api_ele.agr($.htm,$e,'ini');
          }
        });      
      }
      break;
    ...
    default:
      ...
      $_app.tab.lis.querySelectorAll($_app.tab.cla).forEach( $e => api_ele.eli($e,$.eti) );
      ...
      if( $dat.value ){
        ...
        $_app.tab.lis.querySelectorAll($_app.tab.cla).forEach( $e =>{
          ...
          if( $.obj = api.dat($.esq,$.est,$e.dataset[`${$.esq}_${$.est}`]) ){
